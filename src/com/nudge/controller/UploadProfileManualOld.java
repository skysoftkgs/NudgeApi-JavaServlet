 ...
 */
@WebServlet("/UploadProfileManualOld")
public class UploadProfileManualOld extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			for(FileItem item: multiparts)
			{
				...
				else if(item.getFieldName().equals("budget"))
				{
			...
			if(user_id.length()!=0 && name.length()!=0 && relationship.length()!=0 && gender.length()!=0 && dob.length()!=0 && budget.length()!=0 && location.length()!=0 && last_name.length()!=0)
			{
				...
				if(checkresponse)
				{
					...
					if(add_response!=0)
					{
						...
						if(relationship.equalsIgnoreCase("17") || relationship.equalsIgnoreCase("18") )
						{
							...
							// To convert timestamp to date
							String nudgeDateTimestampToDate_Val= Util.MillisecondToDateNew(String.valueOf(nudge_timestamp));
