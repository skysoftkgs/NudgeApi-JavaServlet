...
/**
 * Servlet implementation class RegistrationV1
 */
@WebServlet("/RegistrationV1")
public class RegistrationV1 extends HttpServlet {
		...
		try {
			...
			for(FileItem item: multiparts)
			{
				...
				else
				{
					...
					else if(item.getFieldName().equals("device_type"))
					{
						device_type=item.getString();
					}
					else if(item.getFieldName().equals("device_token"))
					{
						device_token=item.getString();
					}
			...
			if(login_type.equalsIgnoreCase("email") )
			{
				...
				if(name.length()!=0 && email.length()!=0 && password.length()!=0 && dob.length()!=0 && login_type.length()!=0 && unique_id.length()!=0 && location.length()!=0 && last_name.length()!=0 && device_token.length()!=0  && device_type.length()!=0 && gender.length()!=0)
				{
					...
					else
					{
						...
						obj_bean.setDevice_type(device_type);
						obj_bean.setDevice_token(device_token);
						...
						//	UserDetailsBean bean_obj = model_obj.GetUserDetailsBean(email, con);
							...
							String NewMailBody1 = MailFormat.EmailVerMailFormat(verify_url,LOGO,email);

							
							SendMailNew.SendMailUser(NewMailBody1, userobj.getString("email"), EMAIL_VER_SUBJECT,MAIL_FROM,PASSWORD);
